...
const DashboardPortfolio = () => {
  return (
    <div
      ...
      className="detailContainer w-full mt-2 flex md:flex-col"
    >
      <div className="leftpart backdrop-blur-sm  w-[75%] md:w-full border-r-2 border-[#0E0E0E] px-14 air:px-4 py-8">
      ...
      <div className="rightPart w-[25%] py-8 px-4 sm:py-0 md:w-full">
        <div className="w-full flex items-center py-4 justify-center flex-col">
          <p className=" font-lw text-[#292D32] text-[12px] text-start place-self-start">
            Name:
          </p>
          {nodeObject.map((item) => {
            return (
              <div
                key={Math.random()}
                className="w-full bg-black px-2 py-3 my-2 flex items-center gap-4 border-2 border-[#0E0E0E]"
              >
